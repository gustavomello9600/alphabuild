...
class Sensitivity():
    def __init__(self, comm, opt, problem, u_field, lambda_field, rho_phys):
        self.opt_compliance = None # Safe init
        
        # Compliance
        if opt["opt_compliance"]:
            self.C_form = form(opt["compliance"])
        self.dCdrho_form = form(-ufl.derivative(opt["compliance"], rho_phys))
        # self.dCdrho_vec = create_vector(self.dCdrho_form) # Deprecated
        self.dCdrho_vec = assemble_vector(self.dCdrho_form)
        ...
        # self.dVdrho_vec = create_vector(dVdrho_form) # Deprecated
        # assemble_vector(self.dVdrho_vec, dVdrho_form)
        self.dVdrho_vec = assemble_vector(dVdrho_form)
    ...
    def __del__(self):
        if self.opt_compliance is not None and not self.opt_compliance:
            if hasattr(self, 'prod_vec'):
