...
import numpy as np
import ufl
# from dolfinx import la # Deprecated
...
class DensityFilter():
    def __init__(self, comm, rho, rho_tilde, R=1.0, petsc_options={}):
        ...
        self.vec_s0 = rho.x.petsc_vec.copy()
        self.vec_s = rho_tilde.x.petsc_vec.copy()
...
class Heaviside():
    ...
    def forward(self, beta, eta=0.5):
        ...
        self.drho = beta*(1-np.tanh(beta*(self.rho_phys.x.petsc_vec.array-eta))**2) / denominator
        self.rho_phys.x.petsc_vec.array[:] = (
            np.tanh(beta*eta)+np.tanh(beta*(self.rho_phys.x.petsc_vec.array-eta))) / denominator
