...
# from .utility import ... # No imports from fenitop in optimize.py usually, checking...
def optimality_criteria(rho, rho_min, rho_max, V, dCdrho, dVdrho, move=0.05):
    ...
    while ub-lb > 1e-4:
        ...
        # Protect against positive sensitivity (should be negative for compliance)
        # If dCdrho > 0, term is negative -> sqrt(NaN). 
        # We clip -dCdrho to be at least 0.
        sensitivity_term = -dCdrho / (dVdrho + 1e-12)
        sensitivity_term = np.maximum(sensitivity_term, 1e-15) # Ensure positive for sqrt
        
        rho_new = np.maximum.reduce([np.minimum.reduce(
            [rho*(sensitivity_term/mid)**0.5, rho+move, rho_max]), rho-move, rho_min])
