...
class LinearProblem:
    def __init__(self, u, lam, lhs, rhs, l_vec, spring_vec, bcs=[], petsc_options={}):
        ...
        # Safe initialization for __del__
        self.solver = None
        self.lhs_mat = None
        self.rhs_vec = None
        self.spring_vec = None
        self.l_vec = None
        
        # Initialization
        self.u, self.lam = u, lam
        # Use .x.petsc_vec for dolfinx v0.8+
        ...
        # In dolfinx v0.8, assemble_vector(form) creates and returns the vector
        self.rhs_vec = assemble_vector(self.rhs_form)
        ...
        # assemble_vector already assembled it, just update ghosts and apply BCs
    ...
    def __del__(self):
        # self.solver.destroy() # Python GC should handle this usually, but explicit destroy is fine if object exists
        if hasattr(self, 'solver') and self.solver is not None:
            self.solver.destroy()
        if hasattr(self, 'lhs_mat') and self.lhs_mat is not None:
            self.lhs_mat.destroy()
        if hasattr(self, 'rhs_vec') and self.rhs_vec is not None:
            self.rhs_vec.destroy()
        # self.u_wrap.destroy() # Do not destroy Function.vector
        # self.lam_wrap.destroy() # Do not destroy Function.vector
        if hasattr(self, 'spring_vec') and self.spring_vec is not None:
            self.spring_vec.destroy()
        if hasattr(self, 'l_vec') and self.l_vec is not None:
            self.l_vec.destroy()
...
class Communicator():
    ...
    def bcast(self, func, global_values):
        ...
        # Use petsc_vec for consistency (excludes ghost DOFs)
        local_size = func.x.petsc_vec.getLocalSize()
        if local_size != len(self.idx):
            raise ValueError(f"Mismatched sizes: local_size={local_size}, idx_len={len(self.idx)}")
    ...
    def gather(self, func):
        ...
        if type(func) is Function:
            # Use petsc_vec.array which excludes ghost DOFs (only local DOFs)
            values_gather = self.comm.gather(func.x.petsc_vec.array.copy(), root=0)
        elif type(func) is PETSc.Vec:
            values_gather = self.comm.gather(func.array.copy(), root=0)
