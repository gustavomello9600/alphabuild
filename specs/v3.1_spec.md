...
**⚡ Requisito de Performance:** O código de augmentation deve ser altamente performático para não se tornar gargalo durante o treino no Kaggle (GPUs T4/P100). Diretrizes:

*   **Operações Vetorizadas:** Usar NumPy/PyTorch broadcasting ao invés de loops Python.
*   **Pré-alocação:** Evitar criação de arrays temporários dentro do loop de augmentation.
*   **Erosão/Dilatação:** Usar `scipy.ndimage` com `structure` pré-computado ou `torch.nn.functional` com kernels fixos.
*   **Target:** Augmentation completo de um sample em **< 5ms** (200+ samples/segundo).
*   **Sem I/O:** Nenhuma operação de disco durante augmentation.
