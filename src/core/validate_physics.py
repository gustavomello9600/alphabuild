...
from src.core.physics_model import initialize_cantilever_context, PhysicalProperties
from src.core.solver import solve_topology

def main():
    print("=== AlphaBuilder Physics Core Smoke Test ===")
    
    # 1. Configuração
    # Resolução baixa para teste rápido
    resolution = (32, 16) 
    # Propriedades
    # E_solid = 1.0 (adimensional)
    # Carga P = -100.0 (definido no solver)
    # L = 2.0, H = 1.0, b = 1.0 (espessura unitária implícita em 2D Plane Stress/Strain?)
    # Momento de Inércia I = b*h^3 / 12 = 1.0 * 1.0^3 / 12 = 1/12
    # Deflexão Teórica (Bernoulli-Euler): delta = P * L^3 / (3 * E * I)
    
    P = 100.0 # Magnitude da carga
    L = 2.0
    E = 1.0
    I = 1.0**3 / 12.0
    ...
    props = PhysicalProperties(
        E_solid=E,
        E_void=1e-6,
        nu=0.3, # Poisson afeta um pouco em 2D vs Beam Theory 1D, mas deve ser próximo
        disp_limit=5000.0 # Alto para não penalizar fitness no teste
    )
    ...
    if error_pct < 15.0: # Aceitar até 15% de erro devido a efeitos de Poisson e malha grossa
        print("RESULT: PASS")
    else:
    ...
    # Verificar linearidade aproximada
